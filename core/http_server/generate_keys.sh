...
openssl req -x509 -new -nodes -key rootCA-key.pem -subj "/C=CN/ST=BeiJing/O=Micro View/CN=rootca" -sha256 -days 1024 -out rootCA-crt.pem
cd ..
...
openssl req -new -sha256 -key server-key.pem -subj "/C=CN/ST=BeiJing/O=Micro View/CN=Some ip" -out server-csr.pem
...
openssl req -new -sha256 -key client-key.pem -subj "/C=CN/ST=BeiJing/O=Micro View/CN=Some ip" -out client-csr.pem
