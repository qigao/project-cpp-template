    ...
    bool ClientStart()
    {
        ...
        while (1)
        {
            ...
            switch (choice)
            {
                ...
                case 3:
                    //(5)展示文件列表
                    if (ShowFileList(file_name) == false)
                        break;
                    //(6)选择下载
...
private:
    ...
    bool GetAllHost(std::vector<std::string>& list)
    {
        ...
        // 循环遍历网卡
        for (ifa_cur = ifa; ifa_cur != nullptr; ifa_cur = ifa_cur->ifa_next)
        {
            ip = reinterpret_cast<struct sockaddr_in*>(ifa_cur->ifa_addr);
            mask = reinterpret_cast<struct sockaddr_in*>(ifa_cur->ifa_netmask);
    ...
    //(6.2)线程入口函数: 下载分块
    void RangeDownload(std::string& host, std::string& name, int64_t start,
                       int64_t end, int* res)
    {
        // 组织请求路径和下载路径
    ...
    bool DownloadFile(std::string& file_name)
    {
        ...
        // 6.3多线程下载
        int ret = 1;
        std::vector<std::thread> thr_list(count + 1); // 多线程数组
        ...
        for (int i = 0; i <= count; ++i)
        {
            ...
            // 创建多线程下载,并收入数组回收
            int* res = &res_list[i];
            std::thread thr(&MyClient::RangeDownload, this, host, file_name,
                            start, end, res);
            thr_list[i] = std::move(thr);
        }
        // 4.线程回收
