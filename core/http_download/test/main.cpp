...
public:
    explicit Server(int port = 0)
        : _M_port(port), _M_bound_port(0), _M_received_stop_signal(false),
          _M_server(std::make_shared<httplib::Server>()), _M_listen_thread()
    {
    }
    ...
    void start()
    {
        ...
        std::clog << "bound to " << host << ":" << _M_bound_port << std::endl;
        std::exception_ptr ep;
        _M_listen_thread = std::thread(
            [this, &ep]()
            {
                try
                {
    ...
    void stop()
    {
        ...
        if (_M_listen_thread.joinable())
        {
            ...
            std::clog << "unbound from 0.0.0.0:" << _M_bound_port << std::endl;
        }
        _M_bound_port = 0;
        _M_server = NULL;
    ...
    std::shared_ptr<httplib::Server> getInternalServer() { return _M_server; }

    int getBoundPort() const { return _M_bound_port; }

    int getPort() const { return _M_port; }

    void setPort(int value) { _M_port = value; }
...
bool check_partial_support(httplib::Client& client, std::string const& path,
                           int64_t& content_length)
{
    httplib::Result const& r = client.Head(path);
    ...
    if (!r->has_header("Content-Length"))
        return false;
    content_length =
        sstream_cast<int64_t>(r->get_header_value("Content-Length"));
    return true;
}

void download_partial(httplib::Client& client, std::string const& path,
                      long off, long pos)
{
    ...
    std::cout << "part = " << ranges[0].first << " - " << ranges[0].second
              << std::endl;
...
public:
    Ranger(int64_t total_size, int64_t max_partial_size)
        : _M_max_partial_size(max_partial_size), _M_total_size(total_size),
          _M_remain_size(_M_total_size), _M_offset(0), _M_length(0)
    {
    }
    ...
    int64_t offset() const { return _M_offset; }

    int64_t length() const { return _M_length; }
...
try
{
    ...
    if (!server.getInternalServer()->set_mount_point("/downloads",
                                                     home + "/Downloads"))
    ...
    if (http::check_partial_support(client, path, content_length))
    {
        // download partially
        http::Ranger ranger(content_length, max_partial_size);
        while (ranger.next())
            http::download_partial(client, path, ranger.offset(),
