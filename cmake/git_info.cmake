...
#
# GIT_FAIL_IF_NONZERO_EXIT (OPTIONAL) -- Raise a FATAL_ERROR if any of the git
# commands return a non-zero exit code. This is set to TRUE by default. You can
# set this to FALSE if you'd like the build to continue even if a git command
# fails.
#
# GIT_IGNORE_UNTRACKED (OPTIONAL) -- Ignore the presence of untracked files when
# detecting if the working tree is dirty. This is set to FALSE by default.
#
...
# Check that an optional variable is set, or, set it to a default value. Also
# converts that path to an abspath.
...
set(_state_variable_names
    ...
    GIT_DESCRIBE
    GIT_HEAD_BRANCH
    # >>> 1. Add the name of the additional git variable you're interested in
    # monitoring to this list.
    CMAKE_PROJECT_VERSION)

# Macro: RunGitCommand Description: short-hand macro for calling a git function.
# Outputs are the "exit_code" and "output" variables.
macro(RunGitCommand)
  execute_process(
...
# Function: GetGitState Description: gets the current state of the git repo.
# Args: _working_dir (in)  string; the directory from which git commands will be
# executed.
function(GetGitState _working_dir)

  # This is an error code that'll be set to FALSE if the RunGitCommand ever
  # returns a non-zero exit code.
  set(ENV{GIT_RETRIEVED_STATE} "true")
  set(ENV{CMAKE_PROJECT_VERSION} "${CMAKE_PROJECT_VERSION}")
  ...
  rungitcommand(rev-parse --abbrev-ref ${object})
  if(exit_code EQUAL 0)
    set(ENV{GIT_HEAD_BRANCH} "${output}")
  endif()
...
# Function: GitStateChangedAction Description: this function is executed when
...
# Function: HashGitState Description: loop through the git state variables and
...
# Function: CheckGit Description: check if the git repo has changed. If so,
...
function(CheckGit _working_dir _state_changed)
  ...
  #
  # Update the state to include the SHA256 for the pre-configure file. This
  # forces the post-configure file to be regenerated if the pre-configure file
  # has changed.
...
# Function: SetupGitMonitoring Description: this function sets up custom
...
function(SetupGitMonitoring)
  add_custom_target(
    ...
    COMMAND
      ...
      -DGIT_WORKING_DIR=${GIT_WORKING_DIR} -DGIT_EXECUTABLE=${GIT_EXECUTABLE}
