...
function(_configure_target target_name type)
  ...
  target_sources(${target_name} ${source_scope} ${args_SOURCES})
  target_include_directories(${target_name} ${scope} ${args_INCLUDES})
  target_link_libraries(${target_name} ${scope} ${args_LIBRARIES})
  target_include_system_directories(
    ${target_name} ${scope} ${args_SYSTEM_INCLUDES}
  )
  target_find_dependencies(
    ${target_name} "${scope}_CONFIG" ${args_DEPENDENCIES_CONFIG} ${scope}
    ${args_DEPENDENCIES}
  )
  target_link_system_libraries(${target_name} ${scope} ${args_SYSTEM_LIBRARIES})
  target_compile_definitions(
    ${target_name} ${scope} ${args_COMPILE_DEFINITIONS}
  )
  target_compile_options(${target_name} ${scope} ${args_COMPILE_OPTIONS})
  target_compile_features(${target_name} ${scope} ${args_COMPILE_FEATURES})
endfunction()

function(_set_config_execute_args target_name execute_args)
  set_property(
    TARGET ${target_name} APPEND PROPERTY PROJECT_OPTIONS_EXECUTE_ARGS
                                          ${execute_args}
  )
endfunction()
...
function(add_library_test library test_name)
  ...
  target_link_libraries(${target_name} PRIVATE ${prefixed_configs} ${library})

  _get_configs_execute_args(configs_execute_args ${prefixed_configs})
  add_test(NAME ${target_name} COMMAND ${target_name} ${configs_execute_args}
                                       ${args_EXECUTE_ARGS}
...
function(add_executable_test executable test_name)
  ...
  _get_configs_execute_args(configs_execute_args ${prefixed_configs})
  add_test(NAME ${target_name} COMMAND ${executable} ${configs_execute_args}
                                       ${args_EXECUTE_ARGS}
           WORKING_DIRECTORY ${args_WORKING_DIRECTORY}
  )
