...
the ``tests/rpi3``, ``tests/rpi4``, ``tests/emscripten``,
``tests-rpi4-vcpkg`` directories in the
`repositpry <https://github.com/aminya/project_options/tree/main/tests>`__
for full examples.

The following calls ``enable_cross_compiler`` to enable the
...
``enable_cross_compiler`` auto-detects the following cross-compilers,
when setting ``DEFAULT_TRIPLET``:

-  *mingw-w64*
   `x86_64-w64-mingw32.toolchain.cmake <https://github.com/abeimler/project_options/blob/feature/open-closed-enable-cross-compiler/src/toolchains/x86_64-w64-mingw32.toolchain.cmake>`__
-  *emscripten*
...
-  aarch64-linux-gnu
   `aarch64-linux.toolchain.cmake <https://github.com/abeimler/project_options/blob/feature/open-closed-enable-cross-compiler/src/toolchains/aarch64-linux.toolchain.cmake>`__
-  arm-linux-gnueabi, arm-linux-gnueabihf
   `arm-linux.toolchain.cmake <https://github.com/abeimler/project_options/blob/feature/open-closed-enable-cross-compiler/src/toolchains/arm-linux.toolchain.cmake>`__
...
For (bare-metal) you don’t need/can’t set ``arm-linux``/``arm64-linux``
for vcpkg:

-  arm-none-eabi
   `arm.toolchain.cmake <https://github.com/abeimler/project_options/blob/feature/open-closed-enable-cross-compiler/src/toolchains/arm.toolchain.cmake>`__
...
The option for ``DEFAULT_TRIPLET`` are the similar to `vcpkg
triplets <https://github.com/microsoft/vcpkg/tree/master/triplets/community/>`__
...
macro(enable_cross_compiler)
  ...
  cmake_parse_arguments(
    EnableCrossCompiler "${options}" "${oneValueArgs}" "${multiValueArgs}"
    ${ARGN}
  )
  ...
  if("${_default_triplet}" STREQUAL "")
    if(_cc MATCHES "x86_64(-w64)?-mingw32-[gc]..?"
       OR _cxx MATCHES "x86_64(-w64)?-mingw32-[gc]..?"
    )
      set(_default_triplet "x64-mingw-dynamic")
    elseif(_cc MATCHES "i686(-w64)?-mingw32-[gc]..?"
           OR _cxx MATCHES "i686(-w64)?-mingw32-[gc]..?"
    )
      set(_default_triplet "i686-mingw-dynamic")
    elseif(_cc MATCHES "(gcc-)?arm-linux-gnueabi-[gc]..?"
           OR _cxx MATCHES "(gcc-)?arm-linux-gnueabi-[gc]..?"
    )
      set(_default_triplet "arm-linux")
    elseif(_cc MATCHES "(gcc-)?arm-linux-gnueabihf-[gc]..?"
           OR _cxx MATCHES "(gcc-)?arm-linux-gnueabihf-[gc]..?"
    )
      set(_default_triplet "arm-linux")
  ...
  if("${_default_triplet}" STREQUAL "x64-mingw-dynamic"
     OR "${_default_triplet}" STREQUAL "x64-mingw-static"
  )
    if("${_cc}" STREQUAL "")
  ...
  elseif("${_default_triplet}" STREQUAL "x86-mingw-dynamic"
         OR "${_default_triplet}" STREQUAL "x86-mingw-static"
  )
    if("${_cc}" STREQUAL "")
  ...
  if("${_target_architecture}" STREQUAL "")
    ...
    elseif(_cc MATCHES "emcc" OR _cxx MATCHES "em..")
      ...
      message(
        WARNING
          "if you are using arm cross-compiler, please set DEFAULT_TRIPLET"
      )
  ...
  if(_cc MATCHES "(x86_64|i686)(-w64)?-mingw32-[gc]..?"
     OR _cxx MATCHES "(x86_64|i686)(-w64)?-mingw32-[gc]..?"
  )
    set(MINGW TRUE)
    set(USE_CROSSCOMPILER_MINGW TRUE)
  elseif(_cc MATCHES "emcc" OR _cxx MATCHES "em..")
    set(USE_CROSSCOMPILER_EMSCRIPTEN TRUE)
  elseif(_cc MATCHES "aarch64-linux-gnu-gcc" OR _cxx MATCHES
                                                "aarch64-linux-gnu-g\\+\\+"
  )
  ...
  if(BUILD_SHARED_LIBS)
    ...
    if("${_default_triplet}" STREQUAL "x64-mingw-static"
       OR "${_default_triplet}" STREQUAL "x86-mingw-static"
    )
      message(
        WARNING
          "cross-compiler triplet is set to 'static' but BUILD_SHARED_LIBS is enabled"
      )
    endif()
  else()
    if("${_default_triplet}" STREQUAL "x64-mingw-dynamic"
       OR "${_default_triplet}" STREQUAL "x86-mingw-dynamic"
    )
      set(LIBRARY_LINKAGE "dynamic")
    elseif("${_default_triplet}" STREQUAL "x64-mingw-static"
           OR "${_default_triplet}" STREQUAL "x86-mingw-static"
    )
  ...
  elseif(_cc MATCHES "(gcc-)?aarch64-linux-(gnu-)?[gc]..?"
    ...
    set(USE_CROSSCOMPILER_AARCH64_LINUX TRUE)
  elseif(_cc MATCHES "(gcc-)?arm-none-eabi-[gc]..?"
         OR _cxx MATCHES "(gcc-)?arm-none-eabi-[gc]..?"
  )
    if("${_cross_root}" STREQUAL "")
  ...
  if(NOT "${_cross_root}" STREQUAL "" AND "${_cross_triplet}" STREQUAL "")
    message(
      WARNING "CROSS_ROOT (${_cross_root}) is set, but CROSS_TRIPLET is not"
    )
  endif()
  ...
  if(USE_CROSSCOMPILER_EMSCRIPTEN)
    ...
    if(NOT DEFINED CMAKE_CROSSCOMPILING_EMULATOR)
      set(CMAKE_CROSSCOMPILING_EMULATOR
          "$ENV{EMSDK_NODE};--experimental-wasm-threads"
      )
    endif()
  else()
    if(NOT DEFINED CMAKE_TOOLCHAIN_FILE AND NOT DEFINED
  ...
  elseif(USE_CROSSCOMPILER_EMSCRIPTEN)
    ...
    message(
      STATUS
        "use emscripten cross-compiler emulator: ${CMAKE_CROSSCOMPILING_EMULATOR}"
    )
  else()
  ...
  if(NOT "${CMAKE_TOOLCHAIN_FILE}" STREQUAL "")
    ...
    if(NOT DEFINED VCPKG_CHAINLOAD_TOOLCHAIN_FILE)
      message(
        STATUS
          "Cross-compile Toolchain File (for vcpkg): ${CROSS_TOOLCHAIN_FILE}"
      )
    endif()
  endif()
endmacro()
...
function(get_toolchain_file value)
  ...
  elseif(USE_CROSSCOMPILER_EMSCRIPTEN)
    if(EMSCRIPTEN_ROOT)
      set(${value} ${EMSCRIPTEN_ROOT}/cmake/Modules/Platform/Emscripten.cmake
          PARENT_SCOPE
      )
    else()
      message(
        ERROR
        "EMSCRIPTEN_ROOT is not set, please define EMSCRIPTEN_ROOT (emscripten repo)"
      )
    endif()
  ...
  elseif(USE_CROSSCOMPILER_ARM_LINUX)
    set(${value} ${ProjectOptions_SRC_DIR}/toolchains/arm-linux.toolchain.cmake
        PARENT_SCOPE
    )
  ...
  elseif(USE_CROSSCOMPILER_ARM_NONE)
    set(${value} ${ProjectOptions_SRC_DIR}/toolchains/arm.toolchain.cmake
        PARENT_SCOPE
    )
  elseif(DEFAULT_TRIPLET MATCHES "arm")
    message(
      STATUS
        "Don't forget to provide an cmake-toolchain file (for ${DEFAULT_TRIPLET})"
    )
  endif()
endfunction()
