...
function(git_clone)
  ...
  if(NOT EXISTS "${_fun_REPOSITORY_PATH}" OR "${_fun_FORCE_CLONE}" STREQUAL
                                             "TRUE"
  )
    ...
    get_filename_component(
      _fun_REPOSITORY_PARENT_PATH "${_fun_REPOSITORY_PATH}" DIRECTORY
    )
    ...
    execute_process(
      ...
      WORKING_DIRECTORY "${_fun_REPOSITORY_PARENT_PATH}" COMMAND_ERROR_IS_FATAL
                        LAST
    )
  else()
...
function(git_pull)
  ...
  if(NOT "${_fun_TARGET_REVISION}" STREQUAL "" AND "${REVISION}" STREQUAL
                                                   "${_fun_TARGET_REVISION}"
  )
    message(
      STATUS
        "Skipping pull of ${_fun_REPOSITORY_PATH} because it's already at ${REVISION}"
    )
    return()
  else()
  ...
  git_checkout(
    REPOSITORY_PATH "${_fun_REPOSITORY_PATH}" REVISION
    "${_fun_TARGET_REVISION}"
  )
endfunction()
...
function(git_checkout)
  ...
  execute_process(
    COMMAND "${GIT_EXECUTABLE}" "-c" "advice.detachedHead=false" "checkout"
            "${_fun_REVISION}" WORKING_DIRECTORY "${_fun_REPOSITORY_PATH}"
...
function(
  ...
  string(REGEX MATCH "([a-z]+:\/\/)?(.*\/)?([^/]*)\/(.*)" _matched
               "${INPUT_URL}"
  )
...
function(git_add_remote)
  ...
  if(${_find_index} EQUAL -1)
    ...
    execute_process(
      COMMAND "${GIT_EXECUTABLE}" "remote" "add" "${_fun_REMOTE_NAME}"
              "${_fun_REMOTE_URL}"
      WORKING_DIRECTORY "${_fun_REPOSITORY_PATH}" COMMAND_ERROR_IS_FATAL LAST
    )
    execute_process(
...
function(git_switch_back)
  ...
  if(${IS_DETACHED})
    ...
    git_checkout(
      REPOSITORY_PATH "${_fun_REPOSITORY_PATH}" REVISION "${default_branch}"
    )
  endif()
endfunction()
...
function(git_wait)
  ...
  while(NOT EXISTS "${_fun_REPOSITORY_PATH}/.git/index"
    ...
    message(
      STATUS "Waiting for git lock file...[${counter}/${_fun_TIMEOUT_COUNTER}]"
    )
    execute_process(
      COMMAND ${CMAKE_COMMAND} -E sleep 0.5 COMMAND_ERROR_IS_FATAL LAST
    )
...
function(git_default_branch default_branch)
  ...
  if(${_default_branch_result} EQUAL 0)
    string(REGEX REPLACE "refs/remotes/origin/" "" _default_branch
                         "${_default_branch}"
    )
  else()
    message(
      WARNING
        "Could not get default branch of ${_fun_REPOSITORY_PATH}. Considering it as master"
    )
