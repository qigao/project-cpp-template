...
function(git_clone)
    ...
    if(NOT EXISTS "${_fun_REPOSITORY_PATH}" OR "${_fun_FORCE_CLONE}" STREQUAL
        ...
        execute_process(
            ...
            WORKING_DIRECTORY "${_fun_REPOSITORY_PARENT_PATH}"
    ...
    if(${_find_index} EQUAL -1)
        ...
        execute_process(
            COMMAND "${GIT_EXECUTABLE}" "remote" "add" "${_fun_REMOTE_NAME}"
                    "${_fun_REMOTE_URL}"
            WORKING_DIRECTORY "${_fun_REPOSITORY_PATH}" COMMAND_ERROR_IS_FATAL
                              LAST
        )
        execute_process(
...
function(git_switch_back)
    ...
    if(${IS_DETACHED})
        ...
        git_default_branch(
            default_branch REPOSITORY_PATH "${_fun_REPOSITORY_PATH}"
        )
        git_checkout(
            REPOSITORY_PATH "${_fun_REPOSITORY_PATH}" REVISION
            "${default_branch}"
...
function(git_wait)
    ...
    while(NOT EXISTS "${_fun_REPOSITORY_PATH}/.git/index"
        ...
        message(
            STATUS
                "Waiting for git lock file...[${counter}/${_fun_TIMEOUT_COUNTER}]"
        )
        execute_process(
