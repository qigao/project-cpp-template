...
function(_enable_clang_tidy_setup_cross CXX_FLAGS C_FLAGS)
  ...
  if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
    ...
    string(STRIP "${CLANG_TIDY_CXX_FLAGS_COMPILER_TARGET}"
                 CLANG_TIDY_CXX_FLAGS_COMPILER_TARGET
    )
    set(CLANG_TIDY_CXX_FLAGS_COMPILER_TARGET
        "--target=${CLANG_TIDY_CXX_FLAGS_COMPILER_TARGET}"
    )
  endif()
  ...
  if(CMAKE_C_COMPILER_ID STREQUAL "GNU")
    execute_process(
      ...
      ERROR_VARIABLE CLANG_TIDY_C_FLAGS_COMPILER_DEFAULT COMMAND_ERROR_IS_FATAL
                     ANY
    )
    execute_process(
      COMMAND "${CMAKE_C_COMPILER}" "-dumpmachine"
      OUTPUT_VARIABLE CLANG_TIDY_C_FLAGS_COMPILER_TARGET COMMAND_ERROR_IS_FATAL
                      ANY
    )

    string(STRIP "${CLANG_TIDY_C_FLAGS_COMPILER_TARGET}"
                 CLANG_TIDY_C_FLAGS_COMPILER_TARGET
    )
    set(CLANG_TIDY_C_FLAGS_COMPILER_TARGET
        "--target=${CLANG_TIDY_C_FLAGS_COMPILER_TARGET}"
    )
  endif()
  ...
  if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU" OR CMAKE_C_COMPILER_ID STREQUAL "GNU")
    ...
    if(CMAKE_SYSROOT STREQUAL "")
      ...
      list(APPEND CLANG_TIDY_CXX_FLAGS_COMPILER
           ${CLANG_TIDY_CXX_FLAGS_COMPILER_DEFAULT}
      )
      list(APPEND CLANG_TIDY_C_FLAGS_COMPILER
           ${CLANG_TIDY_C_FLAGS_COMPILER_DEFAULT}
      )
    endif()
  endif()
...
function(_enable_clang_tidy_setup CXX_FLAGS C_FLAGS)
  ...
  if(CMAKE_CXX_STANDARD)
    if(CMAKE_CXX_EXTENSIONS)
      list(APPEND CLANG_TIDY_CXX_FLAGS
           -extra-arg=-std=gnu++${CMAKE_CXX_STANDARD}
      )
    else()
      list(APPEND CLANG_TIDY_CXX_FLAGS -extra-arg=-std=c++${CMAKE_CXX_STANDARD})
    endif()
  ...
  if(CMAKE_CROSSCOMPILING)
    _enable_clang_tidy_setup_cross(
      "${CLANG_TIDY_CXX_FLAGS}" "${CLANG_TIDY_C_FLAGS}"
    )
  endif()
...
macro(enable_clang_tidy CLANG_TIDY_EXTRA_ARGUMENTS)
  ...
  if(CLANGTIDY)
    ...
    if((NOT CMAKE_CXX_COMPILER_ID MATCHES ".*Clang" OR (NOT CMAKE_C_COMPILER_ID
                                                        MATCHES ".*Clang"))
    ...
    if("${CMAKE_CXX_CLANG_TIDY_DRIVER_MODE}" STREQUAL "cl"
       OR "${CMAKE_C_CLANG_TIDY_DRIVER_MODE}" STREQUAL "cl"
    )
      _enable_clang_tidy_setup_cl(
        "${CLANG_TIDY_CXX_FLAGS}" "${CLANG_TIDY_C_FLAGS}"
      )
    else()
      _enable_clang_tidy_setup(
        "${CLANG_TIDY_CXX_FLAGS}" "${CLANG_TIDY_C_FLAGS}"
      )
    endif()

    # C++ clang-tidy
    set(CMAKE_CXX_CLANG_TIDY ${CLANGTIDY} ${CLANG_TIDY_CXX_FLAGS}
                             ${CLANG_TIDY_EXTRA_ARGUMENTS}
    )
    # C clang-tidy
    set(CMAKE_C_CLANG_TIDY ${CLANGTIDY} ${CLANG_TIDY_C_FLAGS}
                           ${CLANG_TIDY_EXTRA_ARGUMENTS}
    )
  else()
...
macro(enable_gcc_analyzer _project_name GCC_ANALYZER_EXTRA_ARGUMENTS)
  ...
  if(NOT CMAKE_C_COMPILER_ID STREQUAL "GNU"
     OR (CMAKE_C_COMPILER_ID STREQUAL "GNU" AND CMAKE_C_COMPILER_VERSION
                                                VERSION_LESS "10")
  )
    message(
      ${WARNING_MESSAGE}
...
macro(target_disable_gcc_analyzer TARGET)
  if(CMAKE_C_COMPILER_ID STREQUAL "GNU")
    ...
    if(_compile_options)
      ...
      set_target_properties(
        ${TARGET} PROPERTIES INTERFACE_COMPILE_OPTIONS
    ...
    if(_compile_options)
      ...
      set_target_properties(
        ${TARGET} PROPERTIES COMPILE_OPTIONS
                             "${_compile_options_no_gcc_analyzer}"
      )
    endif()
  endif()
