...
function(_enable_clang_tidy_setup CXX_FLAGS C_FLAGS)
    ...
    if(CMAKE_C_STANDARD)
        if(CMAKE_C_EXTENSIONS)
            list(APPEND CLANG_TIDY_C_FLAGS
                 -extra-arg=-std=gnu${CMAKE_C_STANDARD}
            )
        else()
            list(APPEND CLANG_TIDY_C_FLAGS -extra-arg=-std=c${CMAKE_C_STANDARD})
        endif()
...
macro(enable_clang_tidy CLANG_TIDY_EXTRA_ARGUMENTS)
    ...
    if(CLANGTIDY)
        ...
        if((NOT CMAKE_CXX_COMPILER_ID MATCHES ".*Clang"
            OR (NOT CMAKE_C_COMPILER_ID MATCHES ".*Clang"))
...
macro(target_disable_gcc_analyzer TARGET)
    if(CMAKE_C_COMPILER_ID STREQUAL "GNU")
        get_target_property(
            _compile_options ${TARGET} INTERFACE_COMPILE_OPTIONS
        )
        if(_compile_options)
            string(
                REGEX
