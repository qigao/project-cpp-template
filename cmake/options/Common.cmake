...
macro(common_project_options ENABLE_COMPILE_COMMANDS_SYMLINK)
    ...
    get_property(
        BUILDING_MULTI_CONFIG GLOBAL PROPERTY GENERATOR_IS_MULTI_CONFIG
    )
    if(BUILDING_MULTI_CONFIG)
        if(NOT CMAKE_BUILD_TYPE)
            ...
            message(TRACE
                    "User chose a specific build type, so we are using that"
            )
        ...
        if(${ENABLE_COMPILE_COMMANDS_SYMLINK})
            ...
            if(WIN32)
                ...
                execute_process(
                    ...
                    ERROR_VARIABLE IS_NONADMINISTRATOR OUTPUT_QUIET
                )
            else()
            ...
            if(IS_NONADMINISTRATOR)
                ...
                add_custom_target(
                    ...
                    DEPENDS ${CMAKE_SOURCE_DIR}/compile_commands.json VERBATIM
                )
                message(
                    STATUS
                ...
                if(_compile_commands_symlink_result EQUAL 0)
                    message(TRACE
                            "compile_commands.json was symlinked to the root."
                    )
                else()
                    message(
                        WARNING
                            "failed to create compile_commands.json symlink to the root."
                    )
                endif()
            endif()
            ...
            if(EXISTS ${GITIGNORE_FILE})
                file(STRINGS ${GITIGNORE_FILE} HAS_IGNORED
                     REGEX "^compile_commands.json"
                )
