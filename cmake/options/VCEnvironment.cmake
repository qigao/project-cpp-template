...
function(is_msvc value)
    ...
    if(MSVC
       ...
       OR (CMAKE_CXX_COMPILER_ID STREQUAL "MSVC" AND CMAKE_C_COMPILER_ID
                                                     STREQUAL "MSVC")
       OR (CMAKE_CXX_COMPILER MATCHES "^cl(.exe)?$" AND CMAKE_C_COMPILER MATCHES
                                                        "^cl(.exe)?$")
    )
    ...
    if(NOT CMAKE_CXX_COMPILER
       ...
       AND NOT CMAKE_C_COMPILER_ID
        ...
        if("$ENV{CXX}" MATCHES "^cl(.exe)?$" AND "$ENV{CC}" MATCHES
                                                 "^cl(.exe)?$"
        )
            set(${value} ON PARENT_SCOPE)
            return()
        ...
        if((DETECTED_CMAKE_CXX_COMPILER_ID STREQUAL "MSVC"
            AND DETECTED_CMAKE_C_COMPILER_ID STREQUAL "MSVC")
        )
            set(${value} ON PARENT_SCOPE)
            return()
        endif()
       ...
       WIN32 AND NOT CMAKE_GENERATOR MATCHES "Visual Studio*"
        ...
        if(${_is_msvc})
            ...
            FetchContent_Declare(
                _msvc_toolchain
                GIT_REPOSITORY
                    "https://github.com/MarkSchofield/WindowsToolchain.git"
                GIT_TAG "v0.9.0"
            )
...
macro(run_vcvarsall)
    ...
    if(MSVC)
        ...
        set(VS_DEBUGGER_ENVIRONMENT
            "PATH=\$(VC_ExecutablePath_${VCVARSALL_ARCH_UPPER});%PATH%"
        )
    ...
    if((MSVC_FOUND OR MSVC) AND "$ENV{VSCMD_VER}" STREQUAL "")
        ...
        if(EXISTS ${VCVARSALL_FILE})
            ...
            if("${VCVARSALL_ERROR}" STREQUAL "" AND NOT "${VCVARSALL_OUTPUT}"
                                                    STREQUAL ""
            )
                ...
                message(
                    WARNING
                        "Failed to parse the vcvarsall output. ${VCVARSALL_ERROR}.\nIgnoring this error"
                )
