...
macro(_run_conan1)
    ...
    if(CONAN_EXPORTED)
        ...
        if(NOT CMAKE_CONFIGURATION_TYPES)
            ...
            message(
                STATUS
                    "Multi-configuration build: '${CMAKE_CONFIGURATION_TYPES}'!"
            )
        ...
        foreach(_install_args IN LISTS _args_INSTALL_ARGS)
            string(REGEX MATCH "--build=.*" _possible_build_arg
                         "${_install_args}"
            )
        ...
        foreach(TYPE ${LIST_OF_BUILD_TYPES})
            ...
            if(_should_detect)
                ...
                set(CONAN_ENV ENV "CC=${CMAKE_C_COMPILER}"
                              "CXX=${CMAKE_CXX_COMPILER}"
                )
            elseif(_args_DEPRECATED_CALL)
                # Derive all conan settings from a conan profile
                set(CONAN_SETTINGS PROFILE ${CONAN_DEFAULT_PROFILE} SETTINGS
                                   "build_type=${TYPE}"
                )
...
macro(_run_conan2)
    ...
    if(NOT EXISTS "${CMAKE_BINARY_DIR}/conan_provider.cmake")
        ...
        file(
            ...
            EXPECTED_HASH
                SHA256=0a5eb4afbdd94faf06dcbf82d3244331605ef2176de32c09ea9376e768cbb0fc
            # TLS_VERIFY ON # fails on some systems
        )
    endif()
    ...
    set(CONAN_HOST_PROFILE "${_args_HOST_PROFILE}"
        CACHE STRING "Conan host profile" FORCE
    )
    set(CONAN_BUILD_PROFILE "${_args_BUILD_PROFILE}"
        CACHE STRING "Conan build profile" FORCE
    )
    set(CONAN_INSTALL_ARGS "${_args_INSTALL_ARGS}"
        CACHE STRING "Command line arguments for conan install" FORCE
    )
...
macro(run_conan)
    ...
    if(_conan_current_version VERSION_LESS "2.0.0")
        set_property(
            ...
            PROPERTY PROJECT_OPTIONS_SHOULD_INVOKE_CONAN1 TRUE
        )
        set_property(
            DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
            PROPERTY PROJECT_OPTIONS_CONAN1_ARGS ${ARGN}
        )
    else()
        _run_conan2(${ARGN})
    endif()
