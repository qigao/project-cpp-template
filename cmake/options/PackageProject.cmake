...
function(get_property_of_targets)
    ...
    cmake_parse_arguments(
        args "${_options}" "${_oneValueArgs}" "${_multiValueArgs}" ${ARGN}
    )
...
function(package_project)
    ...
    cmake_parse_arguments(
        _PackageProject "${_options}" "${_oneValueArgs}" "${_multiValueArgs}"
        "${ARGN}"
    )
    ...
    if("${_PackageProject_CONFIG_EXPORT_DESTINATION}" STREQUAL "")
        set(_PackageProject_CONFIG_EXPORT_DESTINATION
            "${CMAKE_BINARY_DIR}/${_PackageProject_NAME}"
        )
    endif()
    ...
    list(APPEND _PackageProject_PUBLIC_DEPENDENCIES
         ${_PUBLIC_DEPENDENCIES_CONFIG} ${PROPERTY_PUBLIC_DEPENDENCIES}
    )
    ...
    list(APPEND _PackageProject_PRIVATE_DEPENDENCIES
         ${_PRIVATE_DEPENDENCIES_CONFIG} ${PROPERTY_PRIVATE_DEPENDENCIES}
    )
    ...
    foreach(_target ${_targets_list})
        set(_targets_str
            "${_targets_str} ${_PackageProject_NAMESPACE}${_target}"
        )
    endforeach()
...
function(target_include_interface_directories target)
    function(target_include_interface_directory target include_dir)
        ...
        set_or_append_target_property(
            ${target} "PROJECT_OPTIONS_INTERFACE_DIRECTORIES" ${include_dir}
        )

        # Include the interface directory
        get_target_property(_HasSourceFiles ${target} SOURCES)
        if(NOT _HasSourceFiles
        )# header-only library, aka `add_library(<name> INTERFACE)`
...
macro(target_find_dependencies target)
    ...
    macro(_property_for property)
        ...
        set_or_append_target_property(
            ${target} "PROJECT_OPTIONS_${property}_DEPENDENCIES"
            "${args_${property}}"
        )
    endmacro()
