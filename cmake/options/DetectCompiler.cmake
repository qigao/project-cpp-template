...
macro(detect_compiler)
    ...
    execute_process(
        COMMAND
            "${CMAKE_EXECUTABLE}" -S "${ProjectOptions_SRC_DIR}/detect_compiler"
            -B "${CMAKE_CURRENT_BINARY_DIR}/detect_compiler" -G
            "${CMAKE_GENERATOR}" "--log-level=ERROR" "-Wno-dev"
        OUTPUT_QUIET
    )
    ...
    foreach(cache_var ${cache_variables})
        file(STRINGS
             "${CMAKE_CURRENT_BINARY_DIR}/detect_compiler/CMakeCache.txt"
             "DETECTED_${cache_var}" REGEX "^${cache_var}:STRING=(.*)$"
        )
        string(REGEX REPLACE "^${cache_var}:STRING=(.*)$" "\\1"
                             "DETECTED_${cache_var}" "${DETECTED_${cache_var}}"
        )
    endforeach()
endmacro()
