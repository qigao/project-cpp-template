...
std::int64_t RollFormula::evaluate() const
{
    if (formula.index() == 0)
    {
        ...
        for (std::size_t i = 0; i < RS.size(); i++)
        {
            ...
            {
                if (std::get<RollOperation>(RS[i].formula) ==
                    RollOperation::recast)
                {
                    modification = {std::get<RollOperation>(RS[i].formula), 0};
                    RS.erase(RS.begin() + i, RS.begin() + i + 1);
                    i--;
                }
                else if (std::get<RollOperation>(RS[i].formula) ==
                         RollOperation::MZ)
                {
                    if (RS.size() < 2)
                        throw RollFormulaError(
                            "[ERROR w/ modifier] not enough arguments");
                    if (RS[i - 1].formula.index() != 1)
                        throw RollFormulaError(
                            "[ERROR w/ modifier] argument is not int");
                ...
                else
                {
                    if (RS.size() < 3)
                        throw RollFormulaError(
                            "[ERROR w/ operation] not enough arguments");
                    try
                    {
                        RS[i - 2].formula =
                            (std::int64_t)RS[i](RS[i - 2], RS[i - 1]);
                    }
                                     ...
                                     RollFormula const& r) const
{
    if (l.formula.index() == 2 || r.formula.index() == 2)
        throw RollFormulaError(
            "[ERROR w/ operand] left or right operand is an operation");
    switch (std::get<RollOperation>(formula))
    {
        ...
        case RollOperation::cast:
            ...
            static std::uniform_int_distribution<std::int64_t> gen(
                1, r.evaluate());
            ...
            for (std::size_t i = 0; i < countOfRoll; i++)
            {
                ...
                if (modification.operation == RollOperation::recast &&
                    resultOfRoll == r.evaluate())
                    countOfRoll++;
                rolls.insert(std::find_if(rolls.begin(), rolls.end(),
                                          [resultOfRoll](auto val)
                                          { return resultOfRoll <= val; }),
                             resultOfRoll);
            }
            ...
            {
                for (int i = 1; i < rolls.size() && i <= modification.value;
                     i++)
                {
