...
static const float identityMatrix
  [16] = { 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f };
...
void OrthoGraphic(const float l,
  float                       r,
  float                       b,
  const float                 t,
  float                       zn,
...
//
//
static const char *SequencerItemTypeNames[] = { "Camera",
  "Music",
  "ScreenEffect",
  "FadeIn",
  "Animation" };
...
struct GraphEditorDelegate : public GraphEditor::Delegate
{
  ...
  void MoveSelectedNodes(const ImVec2 delta) override
  {
    for (auto &node : mNodes) {
      if (!node.mSelected) { continue; }
      node.x += delta.x;
      node.y += delta.y;
    }
  }

  virtual void RightClick(GraphEditor::NodeIndex nodeIndex,
  ...
  const GraphEditor::Node GetNode(GraphEditor::NodeIndex index) override
  {
    ...
    return GraphEditor::Node{ myNode.name,
  ...
  // Graph datas
  static const inline GraphEditor::Template mTemplates[] = { { IM_COL32(160, 160, 180, 255),
    ...
    { IM_COL32(180, 160, 160, 255),
      ...
      3,
      nullptr,
      Array{ IM_COL32(200, 100, 100, 255),
...
int main(int, char **)
{
  ...
  while (!imApp.Done()) {
    ...
    if (isPerspective) {
      ...
      OrthoGraphic(
        -viewWidth, viewWidth, -viewHeight, viewHeight, 1000.f, -1000.f, cameraProjection);
    }
    ...
    if (ImGui::CollapsingHeader("Zoom Slider")) {
      ...
      ImGui::Image((ImTextureID)(uint64_t)procTexture,
        ImVec2(900, 300),
        ImVec2(uMin, vMin),
        ImVec2(uMax, vMax));
      {
        ImGui::SameLine();
        ImGui::PushID(18);
        ImZoomSlider::ImZoomSlider(
          0.f, 1.f, vMin, vMax, 0.01f, ImZoomSlider::ImGuiZoomSliderFlags_Vertical);
        ImGui::PopID();
      }
    ...
    if (showGraphEditor) {
      ...
      if (ImGui::Button("Fit all nodes")) { fit = GraphEditor::Fit_AllNodes; }
      ImGui::SameLine();
      if (ImGui::Button("Fit selected nodes")) { fit = GraphEditor::Fit_SelectedNodes; }
