...
public:
  tray_and_menu()
  {
    ...
    if (tray_.menu != nullptr) { return; }

    tray_.menu    = new tray_menu[8];
    tray_.menu[0] = { .text = "Hello", .cb = hello_cb, .submenu = nullptr };
    tray_.menu[1] = { .text = "Checked", .cb = toggle_cb, .submenu = nullptr };
    ...
    tray_.menu[4].submenu[0] = {
      .text = "FIRST", .checked = 1, .cb = submenu_cb, .submenu = nullptr
    };
    tray_.menu[4].submenu[1] = { .text = "SECOND", .submenu = new tray_menu[6] };
    tray_.menu[4].submenu[2] = { .text = nullptr, .submenu = nullptr };

    tray_.menu[4].submenu[1].submenu[0] = { .text = "THIRD", .submenu = new tray_menu[4] };
    tray_.menu[4].submenu[1].submenu[1] = { .text = "FOUR", .submenu = new tray_menu[3] };
    tray_.menu[4].submenu[1].submenu[2] = { .text = nullptr, .submenu = nullptr };

    tray_.menu[4].submenu[1].submenu[0].submenu[0] = {
      .text = "7", .cb = submenu_cb, .submenu = nullptr
    };
    tray_.menu[4].submenu[1].submenu[0].submenu[1] = { .text = "-", .submenu = nullptr };
    tray_.menu[4].submenu[1].submenu[0].submenu[2] = {
      .text = "8", .cb = submenu_cb, .submenu = nullptr
    };
    tray_.menu[4].submenu[1].submenu[0].submenu[3] = { .text = nullptr, .submenu = nullptr };
    ...
    if (tray_init(&tray_) < 0) { std::runtime_error("failed to create tray_"); }
  }
  ...
  {
    while (tray_loop(1) == 0) { std::cout << "Iteration" << std::endl; }
  }
