...
namespace Encoding {
  ...
  bool is_path_unreserved(unsigned char c) { return !is_special(c) && c != '?' && c != SEPARATOR; }

  static char to_hex(unsigned value)
  {
    const char hex[] = "0123456789ABCDEF";

    if (value >= 16) throw std::out_of_range("value cannot be converted to hex");
  ...
  std::string decode(const std::string &input)
  {
    ...
    while (current != end) {
      if (*current == '%') {
        if ((end - current) < 2) throw std::invalid_argument("invalid percent encoding!");
  ...
  std::string encode_path(const std::string &native_path,
    char                                     separator,
    bool                                     directory,
    bool (*is_safe)(unsigned char))
  {
    std::string buffer;
    ...
    while (current != end) {
      if (*current == separator) {
        ...
        // Otherwise, if it's an unsafe character, we encode it using
        // percent-escape:
    ...
    if (directory && buffer.back() != SEPARATOR) { buffer += SEPARATOR; }
