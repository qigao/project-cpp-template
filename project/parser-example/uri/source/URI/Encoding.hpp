...
// The following predicates will generate pretty URIs which are not strictly RFC
// compliant. However, they should be compatible with all major web browsers and
// other systems. The URIs will be slightly more human readable. If you don't
// want this behaviour, use is_unreserved for all encoding.

// Characters which can appear within the query portion of a URI without any
// encoding.
bool is_query_unreserved(unsigned char);

// Characters which can appear in the path portion of a URI without any
// encoding.
...
std::string encode_query(IteratorT begin, IteratorT end,
                         bool (*is_safe)(unsigned char) = is_query_unreserved)
{
    std::stringstream buffer;

    if (begin != end)
    {
        buffer << encode(begin->first, is_safe) << '='
               << encode(begin->second, is_safe);
