...
{
  // path.size() returns one past the end, and we want to find the second to
  // last slash.
...
static void
  push(std::string &result, const std::string &path, std::size_t current, std::size_t next)
{
...
bool Path::is_absolute() const { return !value.empty() && value.front() == SEPARATOR; }

bool Path::is_directory() const { return !value.empty() && value.back() == SEPARATOR; }
...
std::ostream &operator<<(std::ostream &output, const Path &path) { return output << path.value; }
}// namespace URI
