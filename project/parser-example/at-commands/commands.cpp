...
bool ok() { return response(RESPONSE_OK); }
...
{
    if ((rssi_threshold_index < 0) ||
        (rssi_threshold_index >= NUM_RSSI_THRESHOLD_TEXT))
    {
        return response(RESPONSE_ERROR_INDEX_OUT_OF_BOUNDS);
    }
...
{
    if ((baseband_bandwidth_index < 0) ||
        (baseband_bandwidth_index >= NUM_LNA_GAIN_TEXT))
    {
        return response(RESPONSE_ERROR_INDEX_OUT_OF_BOUNDS);
    }
...
bool sample_rssi(float duration_seconds) { return ok(); }
