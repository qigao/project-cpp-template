...
TEST_F(CSVReaderTest, TestEmptyFile) {
  auto file_stream = std::make_unique<std::ifstream>("./fixtures/empty.csv", std::ios_base::in | std::ios_base::binary);
  auto reader = Parser::CSVStreamParser<>(std::make_unique<Parser::StreamReader>(*file_stream));
...
TEST_F(CSVReaderTest, TestSimpleCSV) {
  auto file_stream = std::make_unique<std::ifstream>("./fixtures/simple.csv", std::ios_base::in | std::ios_base::binary);
  auto reader = Parser::CSVStreamParser<>(std::make_unique<Parser::StreamReader>(*file_stream));
...
TEST_F(CSVReaderTest, TestQuotedCSV) {
  auto file_stream = std::make_unique<std::ifstream>("./fixtures/simple_quoted.csv", std::ios_base::in | std::ios_base::binary);
  auto reader = Parser::CSVStreamParser<>(std::make_unique<Parser::StreamReader>(*file_stream));
...
TEST_F(CSVReaderTest, TestInvalidCSV) {
  auto file_stream = std::make_unique<std::ifstream>("./fixtures/invalid.csv", std::ios_base::in | std::ios_base::binary);
  auto reader = Parser::CSVStreamParser<>(std::make_unique<Parser::StreamReader>(*file_stream));
  ...
  ASSERT_THROW({
    reader.readLine(results);
  }, Parser::file_parsing_error);
  ASSERT_FALSE(reader.valid());
  ASSERT_FALSE(reader.end());
}

TEST_F(CSVReaderTest, TestMultipleBlocksValid) {
  auto file_stream = std::make_unique<std::ifstream>("./fixtures/short_block.csv", std::ios_base::in | std::ios_base::binary);
  auto reader = Parser::CSVStreamParser<32>(std::make_unique<Parser::StreamReader>(*file_stream));
...
TEST_F(CSVReaderTest, TestMultipleBlocksInvalid) {
  auto file_stream = std::make_unique<std::ifstream>("./fixtures/short_block.csv", std::ios_base::in | std::ios_base::binary);
  auto reader = Parser::CSVStreamParser<16>(std::make_unique<Parser::StreamReader>(*file_stream));
